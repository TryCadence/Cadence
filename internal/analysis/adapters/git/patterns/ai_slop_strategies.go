...
func (s *CommitMessageStrategy) Name() string        { return "commit_message_analysis" }
func (s *CommitMessageStrategy) Category() string    { return "behavioral" }
func (s *CommitMessageStrategy) Confidence() float64 { return 0.6 }
func (s *CommitMessageStrategy) Description() string {
	return "Detects AI-typical commit message patterns and phrasing"
}
...
func (s *CommitMessageStrategy) Detect(pair *git.CommitPair, repoStats *metrics.RepositoryStats) (isSuspicious bool, reason string) {
	...
	// Generic patterns that are suspicious ONLY when multiple appear together.
	// Individual patterns like "initial commit" or "update readme" are very common
	// in legitimate human commits and should not flag on their own.
	...
	// Patterns that are almost always legitimate human behavior — excluded from scoring.
	// "initial commit" is expected on first commits, "update readme" and "update dependencies"
	// are standard maintenance activities.
	...
	// Require 2+ AI patterns OR 3+ generic patterns to flag.
	// A single generic pattern (e.g., "bug fixes") is far too common in human commits
	// to be a reliable signal on its own.
	if aiScore >= 2 || genericScore >= 3 {
	...
	// Combined signal: if both AI and generic patterns appear, lower the bar.
	if aiScore >= 1 && genericScore >= 2 {
		return true, fmt.Sprintf(
			"Suspicious commit message patterns - mixed AI/generic phrasing (AI patterns: %d, generic: %d)",
			aiScore, genericScore,
		)
	}

	words := strings.Fields(msg)
	if len(words) > 10 && (strings.Contains(msg, "implement") || strings.Contains(msg, "functionality")) {
...
func (s *NamingPatternStrategy) Name() string        { return "naming_pattern_analysis" }
func (s *NamingPatternStrategy) Category() string    { return "pattern" }
func (s *NamingPatternStrategy) Confidence() float64 { return 0.7 }
func (s *NamingPatternStrategy) Description() string {
	return "Detects generic or AI-typical variable and function naming"
}
...
func (s *StructuralConsistencyStrategy) Name() string        { return "structural_consistency_analysis" }
func (s *StructuralConsistencyStrategy) Category() string    { return "statistical" }
func (s *StructuralConsistencyStrategy) Confidence() float64 { return 0.6 }
func (s *StructuralConsistencyStrategy) Description() string {
	return "Detects suspiciously balanced addition/deletion ratios"
}
...
func (s *BurstPatternStrategy) Name() string        { return "burst_pattern_analysis" }
func (s *BurstPatternStrategy) Category() string    { return "behavioral" }
func (s *BurstPatternStrategy) Confidence() float64 { return 0.7 }
func (s *BurstPatternStrategy) Description() string {
	return "Detects rapid-fire commit patterns suggesting batch processing"
}
...
func (s *ErrorHandlingPatternStrategy) Name() string        { return "error_handling_analysis" }
func (s *ErrorHandlingPatternStrategy) Category() string    { return "pattern" }
func (s *ErrorHandlingPatternStrategy) Confidence() float64 { return 0.6 }
func (s *ErrorHandlingPatternStrategy) Description() string {
	return "Detects missing or excessive error handling typical of AI code"
}
...
func (s *TemplatePatternStrategy) Name() string        { return "template_pattern_analysis" }
func (s *TemplatePatternStrategy) Category() string    { return "pattern" }
func (s *TemplatePatternStrategy) Confidence() float64 { return 0.7 }
func (s *TemplatePatternStrategy) Description() string {
	return "Detects template/boilerplate code patterns from AI generation"
}
...
func (s *FileExtensionPatternStrategy) Name() string        { return "file_extension_analysis" }
func (s *FileExtensionPatternStrategy) Category() string    { return "structural" }
func (s *FileExtensionPatternStrategy) Confidence() float64 { return 0.5 }
func (s *FileExtensionPatternStrategy) Description() string {
	return "Detects suspicious bulk file creation patterns"
}
